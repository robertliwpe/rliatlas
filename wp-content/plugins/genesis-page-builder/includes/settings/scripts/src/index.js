...
const Setting = ( props ) => {
	...
	return (
		<div key={ 'key_' + props.setting } className={ classnames(
			"gpb-control-settings-control-settings-entry",
			open ? 'gpb-control-settings-entry-open' : null
		) }>
			<div className="gpb-control-settings-block-setting">
				{ props.setting.title }
			</div>

			<div className="gpb-control-settings-block-permission">
				<RolesList
					roles={ props.roles }
			...
			<div onClick={ () => { toggleOpen( ! open ) } } className="gpb-control-settings-block-toggle"><span className={ classnames( 'dashicons', open ? 'dashicons-arrow-up-alt2' : 'dashicons dashicons-arrow-down-alt2' )}></span></div>
		</div>
	);
}
...
const Settings = ( props ) => {
	...
	return (
		<>
			<div className="gpb-control-settings-block">
				<div>{ props.block.title }</div>
				<div className="gpb-control-settings-entry-header">
					<span>{ __( 'Block Settings Permissions', 'genesis-page-builder' ) }</span>
				</div>
				<div onClick={ () => { toggleOpen( ! open ) } } className="gpb-control-settings-block-toggle"><span className={ classnames( 'dashicons', open ? 'dashicons-arrow-up-alt2' : 'dashicons dashicons-arrow-down-alt2' )}></span></div>
			</div>
			<div className={ classnames( "gpb-control-settings-control-settings", open ? 'gpb-control-settings-control-settings-open' : null )}>
		...
		return (
			<div className="gpb-control-settings">
				<div className="gpb-control-settings-head">
					<div>{ __( 'Block Name', 'genesis-page-builder' ) }</div>
					<div></div>
					<div>
						<span onClick={ () => { this.setState({ allExpanded: true }) } } className={ classnames( "gpb-control-settings-expand-all", ! this.state.allExpanded ? null : 'gpb-hidden' )}>{ __( 'Expand All', 'genesis-page-builder' ) }</span>
						<span onClick={ () => { this.setState({ allExpanded: false }) } } className={ classnames( "gpb-control-settings-collapse-all", this.state.allExpanded ? 'gpb-visible' : null ) }>{ __( 'Collapse All', 'genesis-page-builder' ) }</span>
					</div>
				</div>
